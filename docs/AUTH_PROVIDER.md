...
async fn verify(
    ...
    let secret = headers
        ...
        .unwrap_or("");
    if secret != state.relay_secret {
        return (StatusCode::UNAUTHORIZED, Json(ErrorResponse {
            error: "invalid relay secret".into()
        })).into_response();
    }
    ...
    let result: Option<String> = db.query_row(
        "SELECT subdomains FROM tokens WHERE token_hash = ? AND revoked_at IS NULL",
        [&token_hash],
        |row| row.get(0),
    ).ok();

    match result {
        Some(subdomains_json) => {
            let subdomains: Vec<String> = serde_json::from_str(&subdomains_json)
                .unwrap_or_default();
            Json(VerifyResponse { subdomains }).into_response()
        }
        None => {
...
#[tokio::main]
async fn main() {
    let db = Connection::open("auth.db").unwrap();
    db.execute_batch("
    ...
    let state = Arc::new(AppState {
        db: Mutex::new(db),
        relay_secret: std::env::var("RELAY_SECRET").expect("RELAY_SECRET required"),
    });
